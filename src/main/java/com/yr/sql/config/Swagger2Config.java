...
public class Swagger2Config {
    ...
    public Docket createRestApi() {
        ...
        ticketPar.name("Authorization").description("Bearer token").modelRef(new ModelRef("string"))
                .parameterType("header").required(false).build();
        parameters.add(ticketPar.build());
        return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).select()
                .apis(RequestHandlerSelectors.basePackage("com.yr.sql.web.controller")).paths(PathSelectors.any()).build()
